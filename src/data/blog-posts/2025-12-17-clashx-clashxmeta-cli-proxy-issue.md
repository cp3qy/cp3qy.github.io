...
最近把代理工具从 ClashX 换成了 ClashX.Meta，结果发现：浏览器能正常科学上网，但 `git push` 卡住，`CLI AI agent` 也无法联网。
...
- ❌ 但在终端里 `git push` 一直超时；
- ❌ `CLI AI agent` 报 API 超时错误。

---

## 二、排查 7891 端口监听状态

ClashX.Meta 的设置里写着 `Proxy Port: 7891`，先排查此端口的监听状态。打开终端输入：

```bash
netstat -an | grep 7891
```

没有任何输出，说明 ClashX.Meta 没有在 7891 端口上监听。进一步排查是不是旧版 ClashX 没清干净，输入：
...
`(CLOSED)` 表示这是一个已经断开的连接尝试，不是服务在运行。再用 `netstat` 检查 `LISTEN` 状态，确认 7890 是空的。

结论：问题不在旧软件残留，而在 ClashX.Meta 没把 7891 端口真正跑起来。
...
用的是同一个远程配置文件，在 ClashX 里可以通过 GUI 直接改端口，但换了 ClashX.Meta 之后，设置里看到 `Proxy Port: 7891`，可终端连不上，Proxifier 也报错。排查后才发现：ClashX.Meta 没有在 7891 上监听。
...
也就是：如果你用的是自己导入的配置文件，在 GUI 里改端口不起作用。ClashX.Meta 只认配置文件里写的 `port` 和 `mixed-port`。
...
但 macOS 的「系统代理」仍被设置为 `7891`，导致系统请求被发送到一个未监听的端口，自然无法建立连接。
...
`mixed-port`（默认 7893）是 Clash 的"混合端口"，同时支持 HTTP 和 SOCKS5。接下来就是让各个工具都走这个端口。
